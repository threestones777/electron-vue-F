    ...
    <div class="salesLeads">
        ...
        <div class="main-table">
            ...
            <el-form :inline="true" :model="formServe" class="search">
                ...
                <el-form-item>
                    <el-input placeholder="请输入管理员id" @input="search" v-model="keywords" style="width:70%;margin-right:10px" size="small">
            ...
            <!-- 新增弹出框 -->
            <el-dialog width="520px" title="销售机会" :visible.sync="dialogServeAdd">
                ...
                <el-form :model="formServeAdd" ref="formAdd">
                    <el-form-item label="机会名称：">
                        <el-input style="width:220px" :disabled="isShow" v-model="formServeDetail.name"></el-input>
                    </el-form-item>
                    ...
                    <el-row>
                        ...
                        <el-col :span="12">
                            <el-form-item label="机会来源：" prop="lesds_from" :rules="rule">
                                <el-select :disabled="isShow" v-model="formServeAdd.lesds_from" placeholder="请选择机会来源">
                                    <el-option
                    ...
                    <el-form-item label="机会描述：" prop="lesds_desc" :rules="rule" class="lesds_desc">
                        <el-input
                        :disabled="isShow"
                        type="textarea"
                        autosize
                        style="width:80%"
                        placeholder="机会描述"
                        v-model="formServeAdd.lesds_desc">
                        </el-input>
                    </el-form-item>
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="预计成交时间：" prop="estimated_time" :rules="rule">
                                <el-date-picker
                                v-model="formServeAdd.estimated_time"
                                type="date"
                                :disabled="isShow"
                        ...
                        <el-col :span="12">
                            <el-form-item label="预计成交金额：" prop="estimated_amount" :rules="rule">
                                <!-- <el-input-number :disabled="isShow" :controls="false" v-model="formServeAdd.estimated_amount" :precision="2" :min="0"></el-input-number> -->
                                <el-input :disabled="isShow" v-model="formServeAdd.estimated_amount" autocomplete="off"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="机会阶段：" prop="lesds_stage" :rules="rule">
                                <el-select :disabled="isShow" v-model="formServeAdd.lesds_stage" placeholder="请选择机会阶段">
                                    <el-option
                        ...
                        <el-col :span="12">
                            <el-form-item label="成交可能性(%)：" class="slider" prop="possibility" :rules="rule">
                                <el-slider :disabled="isShow" v-model="formServeAdd.possibility"></el-slider>
                            </el-form-item>
                        </el-col>
                    </el-row>
            ...
            <!-- 详情弹出框 -->
            <el-dialog width="450px" title="销售机会" :visible.sync="dialogServeDetail">
...
export default {
  ...
  methods: {
    search() {
      ...
      getSalesleadsList(data).then(res => {
        console.log(res);
        if (res.errno == 0) {
    ...
    initData(obj) {
      ...
      getSalesleadsList(this.$qs.stringify(data)).then(res => {
        console.log(res);
        if (res.errno == 0) {
          this.Data = res.data.orders;
          this.total = Number(res.data.record_count);
    ...
    delCmt(v) {
      ...
      this.$confirm("即将执行删除操作,评论及回复都会删除,确认删除吗?", "提示", {
        ...
        .then(() => {
          ...
          delSalesleadsCmt(tmpData).then(res => {
            if (res.errno == 0) {
              this.$message({
                type: "success",
                message: "删除成功!",
    ...
    open3(v) {
      ...
      this.$prompt("请输入回复内容", null, {
        ...
        .then(({ value }) => {
          ...
          console.log(data);
          replySalesleadsCmt(this.$qs.stringify(data)).then(res => {
            if (res.errno == 0) {
              this.$message({
                type: "success",
                message: "成功!"
    ...
    showCmt() {
      ...
      getSalesleadsCmtList(data).then(res => {
        console.log(res);
    ...
    showDetails(row) {
      ...
      getSalesleadsDetail(data).then(res => {
        if (res.errno == 0) {
          console.log(res);
          this.formServeAdd = res.data[0];
          this.formServeAdd.possibility = parseInt(res.data[0].possibility)
        }
      });
    },
    deleteRow(rows) {
      ...
      this.$confirm("即将执行删除操作, 确认删除吗?", "提示", {
        ...
        .then(() => {
          ...
          editSalesleads(tmpData).then(res => {
              console.log(res)
            if (res.errno == 0) {
              this.$message({
                type: "success",
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
            if(this.isEdit) {
                ...
                console.log("222")
                let tmpData = this.$qs.stringify(this.formServeAdd);
                addSalesleads(tmpData).then(res => {
                  if (res.errno == 0) {
                    this.$message({
                      type: "success",
                      message: "添加成功!",
...
.salesLeads >>> .el-form-item.slider{
    display: flex;
    flex-direction: column;
    justify-content: space-between;
    /* position: static; */
}
.salesLeads >>> .el-form-item.slider .el-form-item__label{
